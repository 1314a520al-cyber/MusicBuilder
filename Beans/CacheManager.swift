...
enum CacheManager {
    ...
    static func categories() -> [CacheCategory] {
        ...
        // 小说缓存
        let novelCache = directorySize(in: .cachesDirectory, subpath: "novels")
        // 漫画缓存
        let comicCache = directorySize(in: .cachesDirectory, subpath: "comics")
        // 有声书缓存
        let audiobookCache = directorySize(in: .cachesDirectory, subpath: "audiobooks")
        // 本地导入小说
        let localNovels = directorySize(in: .documentDirectory, subpath: "LocalNovels")
        // 本地导入漫画
        let localComics = directorySize(in: .documentDirectory, subpath: "LocalComics")
        // 其他缓存
        ...
        return [
            ...
            CacheCategory(name: "小说缓存", icon: "book.fill", size: novelCache, color: .indigo),
            CacheCategory(name: "漫画缓存", icon: "photo.on.rectangle", size: comicCache, color: .orange),
            CacheCategory(name: "有声书缓存", icon: "headphones", size: audiobookCache, color: .mint),
            CacheCategory(name: "本地小说", icon: "doc.text", size: localNovels, color: .purple),
            CacheCategory(name: "本地漫画", icon: "doc.richtext", size: localComics, color: .red),
        ]
    }
    ...
    static func clearCategory(_ name: String) {
        ...
        case "小说缓存":
            removeSubpath(in: .cachesDirectory, subpath: "novels")
        case "漫画缓存":
            removeSubpath(in: .cachesDirectory, subpath: "comics")
        case "有声书缓存":
            removeSubpath(in: .cachesDirectory, subpath: "audiobooks")
        case "本地小说":
            removeSubpath(in: .documentDirectory, subpath: "LocalNovels")
        case "本地漫画":
            removeSubpath(in: .documentDirectory, subpath: "LocalComics")
        default: break
        }
    }
