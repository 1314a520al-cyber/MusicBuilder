...
final class KugouMusicAPI {
    ...
    private func officialWebTopLists(limit: Int) async throws -> [KugouTopInfo] {
        ...
        let rows = Self.regexMatches(pattern, in: html).compactMap { groups -> KugouTopInfo? in
            ...
            return KugouTopInfo(
                id: id,
                name: groups.count > 1 ? Self.clean(Self.htmlDecode(groups[1])) : "",
    ...
    private func officialWebPlaylists(limit: Int) async throws -> [Playlist] {
        ...
        let rows = Self.regexMatches(pattern, in: html).compactMap { groups -> Playlist? in
            ...
            return Playlist(
                id: id,
                name: groups.count > 2 ? Self.clean(Self.htmlDecode(groups[2])) : "",
                coverURL: URL(string: cover),
                trackCount: 0,
                source: .kugou
            )
    ...
    // MARK: - 收藏（红心）
    /// 酷狗收藏 / 取消收藏歌曲（移动端接口，本地收藏优先，云端同步尽力而为）
    func like(song: Song, liked: Bool) async throws -> Bool {
        let auth = KugouMusicAuth.shared
        guard auth.isLoggedIn else { return false }
        let hash = song.kugouHash ?? ""
        let albumAudioId = song.kugouAlbumAudioId ?? ""
        guard !hash.isEmpty || !albumAudioId.isEmpty else { return false }
        // 酷狗移动端收藏接口：v2/collect/add（添加）/ v2/collect/del（删除）
        let path = liked ? "/v2/collect/add" : "/v2/collect/del"
        let dataBody: [String: Any] = [
            "hash": hash,
            "album_audio_id": albumAudioId,
            "album_id": song.kugouAlbumId ?? "",
            "songname": song.name,
            "singername": song.artists,
            "type": 1,
        ]
        do {
            let json = try await cloudlistRequest(path, params: [:], data: dataBody)
            let code = Self.deepInt(json, names: ["error_code", "errcode", "code", "status"])
            BeansLogger.shared.log("酷狗收藏\(liked ? "添加" : "取消")：\(song.name) code=\(code)", level: .debug)
            return code == 0 || code == 200
        } catch {
            BeansLogger.shared.log("酷狗收藏接口失败：\(error.localizedDescription)", level: .debug)
            return false
        }
    }
    ...
    private static func mapTrack(_ raw: [String: Any]) -> Song? {
        ...
        if !filename.isEmpty {
            ...
            if parts.count >= 2 {
                if artist.isEmpty, let first = parts.first { artist = clean(first) }
