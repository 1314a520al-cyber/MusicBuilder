...
final class PlayerManager: NSObject, ObservableObject {
    ...
    @Published var useEffectsEngine = false
    private let effectsEngine = EffectAudioEngine.shared
    private var effectsDownloadTask: URLSessionDataTask?
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        // 音效引擎：非默认音效时下载到本地后通过 AVAudioEngine 播放，获得真实EQ/混响效果
        let fx = AudioFxStore.shared
        let effectsActive = fx.bass > 0.01 || fx.reverbIndex > 0 || fx.presetIndex > 0
        if effectsActive, !url.isFileURL {
            setupPlayerWithEffects(url: url)
            return
        }
        useEffectsEngine = false
    ...
    // MARK: - 音效引擎播放
    private func setupPlayerWithEffects(url: URL) {
        useEffectsEngine = true
        isBuffering = true
        loadFailed = false
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("fx_\(UUID().uuidString).m4a")
        effectsDownloadTask?.cancel()
        effectsDownloadTask = URLSession.shared.dataTask(with: url) { [weak self] data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    ...
                    self.playbackConfirmed = true
                    let fx = AudioFxStore.shared
                    self.effectsEngine.applyEQ(preset: fx.presetName, bassBoost: Float(fx.bass))
                    self.effectsEngine.applyReverb(index: fx.reverbIndex)
                    self.effectsEngine.onEnded = { [weak self] in
                        self?.playNext()
                    }
                    self.effectsEngine.play(url: tempURL, rate: Float(self.rate))
                    self.isPlaying = true
