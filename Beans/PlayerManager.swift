...
final class PlayerManager: NSObject, ObservableObject {
    ...
    func seek(to seconds: Double) {
        ...
        // ★ 只改显示进度progress，UI/歌词/进度条立刻跳到目标位置
        // 实际播放时间actualTime由时间观察者独立更新，互不干扰
        progress = clamped
        clock.forceUpdate(progress: clamped)  // 强制更新歌词时钟，确保歌词立即跳到目标位置
        lastSeekTime = Date()
        ...
        isSeeking = true
        seekLockUntil = Date().addingTimeInterval(8.0)
        // 不暂停！直接seek，让AVPlayer自己处理缓冲
        player?.seek(to: CMTime(seconds: clamped, preferredTimescale: 600)) { [weak self] finished in
            guard let self else { return }
            if finished {
                // seek成功：actualTime会被时间观察者更新，接近target时自动同步
                self.isSeeking = false
                self.seekLockUntil = Date.distantPast
            }
            // 失败：保持isSeeking，8秒后由超时解除，progress保持用户选择的值
        }
        // 8秒后强制解除（不管seek结果）
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        timeObserver = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.2, preferredTimescale: 600), queue: .main) { [weak self] time in
            ...
            // ★ 关键：时间观察者只更新actualTime（实际播放时间）
            // 不直接修改progress（显示进度），防止回退
            guard time.seconds.isFinite else { return }
            self.actualTime = time.seconds
            
            // seek期间或锁定期间：不同步显示进度（保持用户选择的位置）
            guard !self.isSeeking && Date() > self.seekLockUntil else { return }
            
            // 如果有seekTarget，只有actualTime接近目标时才同步显示
            if let target = self.seekTarget {
                let diffToTarget = abs(time.seconds - target)
                if diffToTarget < 2.0 {
                    // 实际播放到目标附近了，开始同步显示
                    self.seekTarget = nil
                ...
                // 还没到目标：progress保持用户选择的值，不回退
                return
            }
            
            // 正常播放：同步显示进度（差异<3秒才更新，防止跳变）
            let diff = abs(time.seconds - self.progress)
            if diff <= 3.0 && abs(time.seconds - self.lastPublishedProgress) >= 0.18 {
                self.lastPublishedProgress = time.seconds
                self.progress = time.seconds
            }
