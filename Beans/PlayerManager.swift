...
final class PlayerManager: NSObject, ObservableObject {
    ...
    func seek(to seconds: Double) {
        ...
        // 精确seek，容差为0确保定位准确
        player?.seek(to: CMTime(seconds: clamped, preferredTimescale: 600), toleranceBefore: .zero, toleranceAfter: .zero) { _ in
            // seek完成，时间观察者自动从目标位置更新进度
        }
        updateNowPlaying()
    }
