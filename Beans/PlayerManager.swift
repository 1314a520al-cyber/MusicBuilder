...
final class PlayerManager: NSObject, ObservableObject {
    ...
    private var seekProtectUntil: Date = Date.distantPast
    ...
    func seek(to seconds: Double) {
        ...
        lastSeekTime = Date()
        seekProtectUntil = Date().addingTimeInterval(5.0)  // 5秒保护期
        if usingEffectEngine {
        ...
        // seek前暂停，防止播放中seek被打断
        player.pause()
        // 递归seek，最多重试3次
        func attemptSeek(retry: Int) {
            player.seek(to: CMTime(seconds: clamped, preferredTimescale: 600)) { finished in
                if finished {
                    // seek成功，恢复播放
    ...
    private func startEffectEngine(file: URL) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] timer in
            ...
            // seek后2秒保护期内不覆盖progress
            guard Date() > self.seekProtectUntil else { return }
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        timeObserver = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.2, preferredTimescale: 600), queue: .main) { [weak self] time in
            guard let self, let _ = self.player else { return }
