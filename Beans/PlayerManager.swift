...
final class PlayerManager: NSObject, ObservableObject {
    ...
    func seek(to seconds: Double) {
        ...
        seekTimeoutTimer = Timer.scheduledTimer(withTimeInterval: 10.0, repeats: false) { [weak self] _ in
            self?.isSeeking = false
        }
        // 如果正在播放，先暂停再seek（防止流媒体seek到未缓冲区域时重置到0）
        ...
        player?.seek(
            ...
            // seek失败(finished=false)时不清除isSeeking：保持用户选择的位置不被时间观察者覆盖
            // 由10秒超时机制清除isSeeking
            ...
            if actual > 0.5 && abs(actual - clamped) < 2.0 && abs(actual - self.progress) > 0.25 {
                self.progress = actual
            }
            self.lastPublishedProgress = self.progress
        }
        updateNowPlaying()
    }
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        timeObserver = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.2, preferredTimescale: 600), queue: .main) { [weak self] time in
            ...
            if time.seconds.isFinite {
                // 如果实际时间与当前 progress 差异超过 3 秒，说明可能是 seek 失败/缓冲未到位
                // 此时不覆盖 progress，保持用户选择的位置（歌词和进度条显示用户点击的位置）
                let diff = abs(time.seconds - self.progress)
                if diff > 3.0 {
                    // 只更新 lastPublishedProgress，不覆盖 progress
                    if abs(time.seconds - self.lastPublishedProgress) >= 0.18 {
                        self.lastPublishedProgress = time.seconds
                    }
                } else if abs(time.seconds - self.lastPublishedProgress) >= 0.18 {
                    self.lastPublishedProgress = time.seconds
