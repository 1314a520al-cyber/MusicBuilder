...
final class PlayerManager: NSObject, ObservableObject {
    ...
    private var targetSeekTime: Double = 0
    private var seekPollTimer: Timer?
    private var seekStartTime: Date = Date()
    ...
    func seek(to seconds: Double) {
        ...
        // 用自己的状态机管理seek，不依赖AVPlayer的completion handler（不可靠）
        isSeeking = true
        ...
        // 启动轮询：每0.3秒检查播放器实际时间是否接近目标
        seekPollTimer = Timer.scheduledTimer(withTimeInterval: 0.3, repeats: true) { [weak self] _ in
            guard let self else { return }
            let actual = self.player?.currentTime().seconds ?? 0
            let elapsed = Date().timeIntervalSince(self.seekStartTime)
            // 成功：实际时间接近目标（差异<2秒），或已经播放到目标之后
            if abs(actual - self.targetSeekTime) < 2.0 || actual > self.targetSeekTime {
                self.seekPollTimer?.invalidate()
                self.seekPollTimer = nil
                self.isSeeking = false
                self.progress = actual
                self.lastPublishedProgress = actual
                if wasPlaying { self.player?.play() }
                return
            }
            // 超时15秒：放弃，保持用户选择的位置，恢复播放
            if elapsed > 15 {
                self.seekPollTimer?.invalidate()
                self.seekPollTimer = nil
                self.isSeeking = false
                // progress保持用户选择的targetSeekTime，不被实际时间覆盖
                if wasPlaying { self.player?.play() }
                return
            }
        }
