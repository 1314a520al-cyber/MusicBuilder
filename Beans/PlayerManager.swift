...
final class PlaybackClock: ObservableObject {
    ...
    /// 强制更新（seek时使用，绕过阈值过滤，确保歌词/进度条立即跳到目标位置）
...
final class PlayerManager: NSObject, ObservableObject {
    /// 最后一次 seek 的时间，防止 seek 后 1 秒内误触切歌
    var lastSeekTime: Date = Date(timeIntervalSince1970: 0)
    /// 快进/后退秒数（用户可自定义，默认15秒）
    var seekInterval: Double {
        get { UserDefaults.standard.double(forKey: "beans.seekInterval") > 0 ? UserDefaults.standard.double(forKey: "beans.seekInterval") : 15 }
        set { UserDefaults.standard.set(newValue, forKey: "beans.seekInterval") }
    }
    private var statsTickCount = 0  // 时间观察者计数，每5次=1秒
    /// 实际播放时间（AVPlayer真实位置，仅时间观察者更新）
    private var actualTime: Double = 0
    /// seek后需要同步的目标位置（actualTime接近此值时才同步到progress）
    ...
    func next(manual: Bool = true) {
        // 防误触：seek 后 1 秒内禁止切歌（避免歌词点击/快进按钮被误判为滑动切歌）
        guard Date().timeIntervalSince(lastSeekTime) > 1.0 else { return }
        // 保存当前歌曲播放进度
    ...
    func seek(to seconds: Double) {
        ...
        let clamped = max(0, min(seconds, maxDuration))
        // 立即更新UI
        progress = clamped
        clock.forceUpdate(progress: clamped)
        lastSeekTime = Date()
        if usingEffectEngine {
            EffectAudioEngine.shared.seek(to: clamped)
            // 2.5秒后强制同步一次（确保保护期结束后进度正确）
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) { [weak self] in
                guard let self, self.usingEffectEngine else { return }
                let engineTime = EffectAudioEngine.shared.currentTime
                if abs(engineTime - self.progress) > 1.0 {
                    self.progress = engineTime
                }
            }
            updateNowPlaying()
            return
    ...
    private func loadCurrent() {
        ...
        // seek保护：距离上次seek不到3秒时，可能是误触发切歌，不重置progress
        if Date().timeIntervalSince(lastSeekTime) > 3.0 {
            progress = 0
    ...
    private func startEffectEngine(file: URL) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] timer in
            ...
            // seek后2秒保护期内不覆盖progress（防止EffectAudioEngine返回错误时间导致回退）
            let sinceSeek = Date().timeIntervalSince(self.lastSeekTime)
            if sinceSeek > 2.0 {
                let engineTime = EffectAudioEngine.shared.currentTime
                // 只在引擎时间合理时才更新（避免0值覆盖）
                if engineTime > 0.1 || self.progress < 0.1 {
                    self.progress = engineTime
                }
    ...
    private func setupPlayer(url: URL, thirdPartyVIPNotice: ThirdPartyVIPNotice? = nil) {
        ...
        if let song = currentSong, lastCountedSongID != song.identityKey {
            ...
            // 每日统计（年度报告数据源）
            ListeningStatsTracker.shared.recordPlay(song: song, duration: song.duration)
        ...
        timeObserver = player.addPeriodicTimeObserver(forInterval: CMTime(seconds: 0.2, preferredTimescale: 600), queue: .main) { [weak self] time in
            ...
            if time.seconds.isFinite {
                // 每秒记录到每日统计
                self.statsTickCount += 1
                if self.statsTickCount >= 5 {
                    self.statsTickCount = 0
                    ListeningStatsTracker.shared.addPlaybackSecond()
                }
                // 直接更新进度和歌词时钟
                self.progress = time.seconds
                self.clock.update(progress: time.seconds)
            }
    ...
    private func updateNowPlaying() {
        ...
        if let artworkURL = song.coverURL {
            ...
            if let cached = Self.nowPlayingArtworkCache.object(forKey: artworkURL as NSURL) {
                ...
                let expectedIdentity = song.identityKey
                // 异步下载专辑封面，防陈旧覆盖 + 失败重试
                URLSession.shared.dataTask(with: artworkURL) { [weak self] data, _, error in
                    ...
                    guard let data, let image = UIImage(data: data) else {
                        // 下载失败：重置key以便下次重试
                        DispatchQueue.main.async {
                            if self.lastNowPlayingArtworkKey == artworkKey {
                                self.lastNowPlayingArtworkKey = nil
                            }
                        }
                        return
                    }
                    ...
                    DispatchQueue.main.async {
                        // 防陈旧：只有当前歌曲还是下载时的歌曲才更新
                        guard self.currentSong?.identityKey == expectedIdentity else { return }
