            ...
            HStack(alignment: .center, spacing: 10) {
                ...
                HStack(spacing: 6) {
                    if let imageName = provider.brandImageName {
                        BrandIconView(name: imageName, size: 15)
                    } else {
                ...
                Button {
                    ...
                    HStack(spacing: 6) {
                        if let imageName = p.brandImageName {
                            BrandIconView(name: imageName, size: 14)
                        } else {
    ...
    private func startSearch(_ text: String) async {
        ...
        searchTask = Task {
            ...
            let timeoutTask = Task {
                ...
                if !Task.isCancelled { searchTask?.cancel() }
            }
            defer { if !Task.isCancelled { searching = false }; timeoutTask.cancel() }
            do {
                try? await Task.sleep(nanoseconds: 15_000_000_000)
                if !Task.isCancelled { searchTask?.cancel() }
            }
            do {
                ...
                case (.netease, .song):
                    let songs = try await NetEaseAPI.shared.search(keyword: trimmed, limit: 20)
                    guard !Task.isCancelled else { return }
                    songResults = songs
                ...
                case (.qq, .album):
                    ...
                    albumResults = albums
                case (.kugou, .song):
                    let songs = try await KugouMusicAPI.shared.searchSongs(keyword: trimmed, limit: 20)
                    guard !Task.isCancelled else { return }
                    songResults = songs
                    if !songs.isEmpty { BeansHaptics.success() }
                case (.kugou, .artist):
