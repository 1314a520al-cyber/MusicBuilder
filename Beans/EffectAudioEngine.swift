...
final class EffectAudioEngine: ObservableObject {
    ...
    func applyEQ(preset: String, bassBoost: Float) {
        let gains = eqGains[preset] ?? Array(repeating: Float(0), count: 10)
        for (i, band) in eqNode.bands.enumerated() {
            // 前3个频段（低频）叠加重低音
            let bass = i < 3 ? bassBoost * 8 : 0
            band.gain = gains[i] + bass
            band.bypass = false
            band.bandwidth = 1.0
        }
    }
    ...
    func seek(to time: TimeInterval) {
        ...
        playerNode.stop()
        playerNode.scheduleSegment(audioFile, startingFrame: frame, frameCount: AVAudioFrameCount(audioFile.length - frame), at: nil)
        playerNode.play()
        playerNode.rate = playbackRate
        currentTime = time
        isPlaying = true
