...
final class NetEaseAPI {
    ...
    /// 相似歌曲推荐
    func similarSongs(id: Int, limit: Int = 30) async throws -> [Song] {
        let json = try await request("/api/discovery/recommend/songs", payload: ["limit": limit], crypto: "weapi")
        let rows = (json["recommend"] as? [[String: Any]]) ?? ((json["data"] as? [[String: Any]]) ?? [])
        return rows.compactMap(Song.init(json:))
    }
    ...
    // MARK: - 听歌打卡（同步听歌时长到网易云，升级等级）
    /// 发表评论到网易云
    func postComment(songID: Int, content: String) async -> Bool {
        ...
        do {
            let json = try await request("/api/v1/resource/comments/R_SO_4_\(songID)", payload: payload, crypto: "weapi")
            let code = json["code"] as? Int
            BeansLogger.shared.log("网易云发评论：\(code == 200 ? "成功" : "失败 code=\(code ?? -1)")", level: .info)
            return code == 200
        } catch {
            BeansLogger.shared.log("网易云发评论失败：\(error.localizedDescription)", level: .warn)
            return false
        }
    }

    func scrobble(songID: Int, duration: Int, source: Int = 0) async {
        let log: [String: Any] = [
            "action": "play",
            "json": [
                "id": "\(songID)",
        ...
        guard let logData = try? JSONSerialization.data(withJSONObject: log),
              let logStr = String(data: logData, encoding: .utf8) else { return }
        let payload: [String: Any] = ["logs": "[\(logStr)]"]
        do {
            _ = try await request("/weapi/feedback/weblog", payload: payload, crypto: "weapi")
            BeansLogger.shared.log("网易云听歌打卡成功：歌曲\(songID)，时长\(duration)s", level: .info)
        } catch {
            BeansLogger.shared.log("网易云听歌打卡失败：\(error.localizedDescription)", level: .warn)
        }
}
}
