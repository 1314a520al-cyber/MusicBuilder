...
struct NovelModuleView: View {
    ...
    @StateObject private var localStore = LocalNovelStore.shared
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // 分类标签
                    // 本地小说
                    if !localStore.novels.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            Text("本地小说（\(localStore.novels.count)）")
                                .font(.headline)
                                .foregroundColor(.primary)
                            ForEach(localStore.novels) { novel in
                                NavigationLink(value: novel) {
                                    HStack(spacing: 12) {
                                        Rectangle()
                                            .fill(Color.blue.opacity(0.2))
                                            .frame(width: 50, height: 70)
                                            .overlay(Image(systemName: "book.fill").foregroundColor(.blue))
                                        ...
                                        VStack(alignment: .leading, spacing: 4) {
                                            Text(novel.title)
                                                ...
                                                .foregroundColor(.primary)
                                                .lineLimit(1)
                                            Text("\(novel.chapters.count)章 · \(novel.wordCount)字")
                                                .font(.caption)
                                                .foregroundColor(.secondary)
                                        }
                                        Spacer()
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Button { showFilePicker = true } label: { Label("导入本地小说", systemImage: "square.and.arrow.down") }
            ...
            .fileImporter(isPresented: $showFilePicker, allowedContentTypes: [.plainText, .text], allowsMultipleSelection: true) { result in
                if case .success(let urls) = result {
                    Task { for url in urls { _ = try? await localStore.importNovel(from: url) } }
                }
            }
            .navigationDestination(for: LocalNovelStore.LocalNovel.self) { novel in
                LocalNovelDetailView(novel: novel)
            }
...
// MARK: - 本地小说详情
struct LocalNovelDetailView: View {
    let novel: LocalNovelStore.LocalNovel
    @StateObject private var localStore = LocalNovelStore.shared
        @StateObject private var theme = ThemeStore.shared
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 16) {
                HStack(alignment: .top, spacing: 16) {
                    Rectangle()
                        ...
                        .overlay(Image(systemName: "book.fill").font(.largeTitle).foregroundColor(.blue))
                        .clipShape(RoundedRectangle(cornerRadius: 8))
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Text(novel.author)
                            ...
                            .foregroundColor(.secondary)
                        HStack {
                            Tag(text: "\(novel.chapters.count)章")
                            Tag(text: "\(novel.wordCount)字")
                        }
                        Text("本地导入")
                            .font(.caption)
                            .foregroundColor(.secondary)
                ...
                NavigationLink {
                    LocalNovelReaderView(novel: novel, currentIndex: novel.lastReadChapter)
                } label: {
                    Text(novel.lastReadChapter > 0 ? "继续阅读（第\(novel.lastReadChapter + 1)章）" : "开始阅读")
                        .font(.headline)
                        .foregroundColor(.white)
                        .frame(maxWidth: .infinity)
                        .padding()
        ...
        .navigationTitle(novel.title)
        .navigationBarTitleDisplayMode(.inline)
        .navigationDestination(for: LocalNovelStore.LocalNovelChapter.self) { chapter in
            LocalNovelReaderView(novel: novel, currentIndex: novel.chapters.firstIndex(of: chapter) ?? 0)
        }
    }
}

// MARK: - 本地小说阅读器
struct LocalNovelReaderView: View {
    let novel: LocalNovelStore.LocalNovel
    let currentIndex: Int
    @State private var currentIdx: Int
    @State private var showControls = false
    ...
    init(novel: LocalNovelStore.LocalNovel, currentIndex: Int) {
        self.novel = novel
        self.currentIndex = currentIndex
        _currentIdx = State(initialValue: currentIndex)
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: settings.lineSpacing) {
                        Text(novel.chapters[currentIdx].title)
                            .font(.title2.bold())
                            .foregroundColor(settings.textColors[settings.textColorIndex])
                            .padding(.bottom, 10)
                        
                        Text(novel.chapters[currentIdx].content)
                            .font(.system(size: CGFloat(settings.fontSize)))
                            .foregroundColor(settings.textColors[settings.textColorIndex])
                            .lineSpacing(CGFloat(settings.lineSpacing))
                    }
                ...
                .onTapGesture {
                    withAnimation { showControls.toggle() }
                    localStore.updateProgress(novelId: novel.id, chapter: currentIdx, offset: 0)
                }
                    ...
                    VStack {
                        ...
                        VStack(spacing: 0) {
                            HStack {
                                ...
                                Spacer()
                                Text("\(currentIdx + 1)/\(novel.chapters.count)")
                                    .font(.subheadline)
                                Spacer()
                                
                                Button {
                                    if currentIdx < novel.chapters.count - 1 { currentIdx += 1 }
                                } label: {
                                    Text("下一章")
                                    Image(systemName: "chevron.right")
                                }
                                .disabled(currentIdx == novel.chapters.count - 1)
                                .foregroundColor(currentIdx == novel.chapters.count - 1 ? .gray : .blue)
                            }
                            .padding()
                            ...
                            HStack(spacing: 30) {
                                Button { settings.fontSize = max(12, settings.fontSize - 1) } label: {
                                    VStack { Image(systemName: "textformat.size.smaller"); Text("减小") }
                                }
                                Button { settings.fontSize = min(28, settings.fontSize + 1) } label: {
                                    VStack { Image(systemName: "textformat.size.larger"); Text("增大") }
                                }
                                Button { showSettings = true } label: {
                                    VStack { Image(systemName: "gearshape"); Text("设置") }
                                }
                                Button { settings.autoScroll.toggle() } label: {
                                    VStack { Image(systemName: settings.autoScroll ? "pause.circle" : "play.circle"); Text(settings.autoScroll ? "暂停" : "自动") }
                                }
                            }
                            .font(.caption)
                            .foregroundColor(.primary)
