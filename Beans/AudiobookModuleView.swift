...
struct AudiobookModuleView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .sheet(isPresented: $showSources) { AudiobookSourcesView() }
...
// MARK: - 有声书音源管理
struct AudiobookSourcesView: View {
    @StateObject private var store = AudiobookSourceStore.shared
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("有声书音源（\(store.sources.filter{$0.enabled}.count)个启用）")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("完成") { dismiss() }
