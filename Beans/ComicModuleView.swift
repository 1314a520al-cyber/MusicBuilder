...
struct ComicModuleView: View {
    ...
    @StateObject private var localStore = LocalComicStore.shared
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    // 本地漫画
                    if !localStore.comics.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            HStack {
                                Text("本地漫画（\(localStore.comics.count)）")
                                    .font(.headline)
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: 12) {
                                    ForEach(localStore.comics) { comic in
                                        NavigationLink(value: comic) {
                                            VStack(spacing: 6) {
                                                if let coverURL = URL(string: comic.coverPath), let img = UIImage(contentsOfFile: coverURL.path) {
                                                    Image(uiImage: img)
                                                        .resizable()
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Menu {
                        Button { showFilePicker = true } label: { Label("导入本地漫画", systemImage: "square.and.arrow.down") }
            ...
            .fileImporter(isPresented: $showFilePicker, allowedContentTypes: [.zip, .image], allowsMultipleSelection: true) { result in
                if case .success(let urls) = result {
                    Task { for url in urls { _ = try? await localStore.importComic(from: url) } }
                }
            }
            .navigationDestination(for: LocalComicStore.LocalComic.self) { comic in
                LocalComicReaderView(comic: comic)
            }
...
// MARK: - 本地漫画阅读器
struct LocalComicReaderView: View {
    let comic: LocalComicStore.LocalComic
    @StateObject private var localStore = LocalComicStore.shared
    @StateObject private var settings = ComicSettings.shared
    @State private var currentPage = 0
    @State private var showControls = false
                    ...
                    ScrollView {
                        LazyVStack(spacing: 0) {
                            ForEach(0..<comic.pageCount, id: \.self) { idx in
                                if let url = localStore.imageURL(for: comic, pageIndex: idx),
                        ...
                        ForEach(0..<comic.pageCount, id: \.self) { idx in
                            if let url = localStore.imageURL(for: comic, pageIndex: idx),
                               let img = UIImage(contentsOfFile: url.path) {
                                ...
                                Color.gray.opacity(0.2).tag(idx)
                            }
                        }
                    }
                    .tabViewStyle(.page(indexDisplayMode: .never))
