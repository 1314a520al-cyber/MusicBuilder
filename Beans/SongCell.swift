...
struct SongCell: View {
    ...
    private var rowContent: some View {
        ...
        .contextMenu {
            Button {
                Task {
                    let liked = await FavoritesStore.shared.toggle(song)
                    ToastCenter.shared.show(liked ? "已收藏" : "已取消收藏")
                }
            } label: {
                Label(FavoritesStore.shared.isLiked(song) ? "取消收藏" : "收藏", systemImage: "heart")
            }
            Button {
                Task {
                    ToastCenter.shared.show("开始下载…")
                    let result = await DownloadManager.shared.download(song: song, quality: .high)
                    if case .success(let r) = result {
                        ToastCenter.shared.show(r.downgraded ? "已下载（音质已自动降级）" : "已下载，可在播放页保存")
                        _ = r.url
                    } else {
                        ToastCenter.shared.show("下载失败，可换个音源再试")
                    }
                }
            } label: {
