...
struct PlayerView: View {
    ...
    @State private var shareCardFileURL: URL?
    ...
    @State private var pendingDownloadQuality: DownloadQuality?
    @ObservedObject private var downloadStore = DownloadStore.shared
    @State private var showShare = false
    @State private var showMoreActions = false
    @State private var showSeekIntervalSetting = false
    ...
    @AppStorage("beans.vinylRecordStyle") private var vinylRecordStyle = false
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showQueue) { QueueView().environmentObject(player).environmentObject(favorites).environmentObject(auth) }
        .sheet(isPresented: $showSleepTimer) { SleepTimerSheet().environmentObject(player) }
        .sheet(isPresented: $showEffect) { AudioEffectSheet().environmentObject(player) }
        .sheet(isPresented: $showSimilar) {
            if let song = player.currentSong {
                SimilarSongsView(sourceSong: song).environmentObject(player)
            }
        }
        .sheet(isPresented: $showScene) {
            SceneRecommendView(scene: SimilarSongsManager.currentScene()).environmentObject(player)
        }
        .sheet(isPresented: $showShareCard, onDismiss: {
            // 分享结束后清理，防止重复弹窗
            shareCardFileURL = nil
            isGeneratingShareCard = false
        }) {
            if let url = shareCardFileURL {
        ...
        .sheet(isPresented: $showDownloadFolderPicker) {
            DownloadFolderPickerSheet(pendingQuality: $pendingDownloadQuality) { folderID in
                if let quality = pendingDownloadQuality {
                    Task { await downloadCurrent(quality, folderID: folderID) }
                }
                pendingDownloadQuality = nil
            }
            .environmentObject(player)
        ...
        HStack(spacing: 12) {
            ...
            Button {
                ...
                Image(systemName: "ellipsis.circle")
                    ...
                    .frame(width: 38, height: 38)
                    .background {
                        playerButtonSurface(size: 38, active: showMoreActions)
                    }
                    .clipShape(Circle())
            }
            .buttonStyle(GlassPressButtonStyle())
    ...
    private var playerMoreActionsPanel: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            let items: [(String, String, () -> Void)] = [
                ("定时", player.sleepTimerRemaining > 0 ? "moon.zzz.fill" : "moon.zzz", { showMoreActions = false; showSleepTimer = true }),
                ("歌单", "text.badge.plus", { showMoreActions = false; showAddToPlaylist = true }),
                ("下载", "arrow.down.circle", { showMoreActions = false; showDownloadPicker = true }),
                ("分享", "square.and.arrow.up", { showMoreActions = false; showShare = true }),
                ("本地", "internaldrive", { showMoreActions = false; showAddToLocalPlaylist = true }),
                ("音效", "wand.and.stars", { showMoreActions = false; showEffect = true }),
                ("快进时间", "forward.end.alt.fill", { showMoreActions = false; showSeekIntervalSetting = true }),
                ("相似", "music.note.list", { showMoreActions = false; showSimilar = true }),
                ("情境", "sparkles", { showMoreActions = false; showScene = true }),
                ("分享卡", "square.and.arrow.up.circle", { showMoreActions = false; generateShareCard() }),
                ("设置", "slider.horizontal.3", { showMoreActions = false; showPlayerSettings = true }),
                ("音游", "gamecontroller.fill", { showMoreActions = false; showRhythmGame = true }),
                ("评论", "text.bubble.fill", { showMoreActions = false; showComments = true }),
            ]
                        ...
                        VStack(spacing: 5) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 12, style: .continuous)
                                    .fill(LinearGradient(colors: [controlAccent.opacity(0.25), controlAccent.opacity(0.10)], startPoint: .topLeading, endPoint: .bottomTrailing))
        ...
        .background {
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .fill(LinearGradient(colors: [Color(.systemBackground).opacity(0.92), Color(.systemBackground).opacity(0.82)], startPoint: .top, endPoint: .bottom))
                .overlay(RoundedRectangle(cornerRadius: 24, style: .continuous).strokeBorder(Color.white.opacity(0.12), lineWidth: 0.8))
        }
    ...
    private func albumPanel(geo: GeometryProxy) -> some View {
            ...
            Button {
                ...
                ZStack {
                    ...
                    if vinylRecordStyle {
                        ZStack {
                            Circle()
                                .fill(LinearGradient(colors: [Color(white: 0.12), Color(white: 0.05)], startPoint: .topLeading, endPoint: .bottomTrailing))
                                ...
                                .modifier(CoverSpin(enabled: true, isPlaying: playerVisualsActive))
                            // 中心标签
                            Circle()
                                .fill(LinearGradient(colors: [Color.red.opacity(0.8), Color.orange.opacity(0.6)], startPoint: .top, endPoint: .bottom))
                    ...
                    // 封面（静态）
                    CoverImage(url: song?.coverURL, size: vinylRecordStyle ? size * 0.82 : size, cornerRadius: vinylRecordStyle ? size * 0.41 : coverRadius, emptyHint: player.isBuffering ? "等待开始播放…" : nil)
                        .offset(x: vinylRecordStyle ? -size * 0.05 : 0)
        ...
        .offset(y: swipeOffset + dismissDrag * 0.6)
    ...
    private func lyricsPanel(geo: GeometryProxy) -> some View {
        ...
        // 高优先级拖拽手势：捕获歌词区域内所有滑动，彻底阻断父视图切歌手势
        // minimumDistance:1 确保轻微滑动就被捕获，点击则透传给歌词行
    }
    ...
    private var progressBlock: some View {
        VStack(spacing: 1) {
            ...
            HStack(spacing: 6) {
                seekPillButton("gobackward.15") { player.seekBy(-player.seekInterval) }
                ...
                seekPillButton("goforward.15") { player.seekBy(player.seekInterval) }
            }
        }
    }
    ...
    private func downloadCurrent(_ quality: DownloadQuality, folderID: UUID? = nil) async {
        guard let song else { return }
        BeansHaptics.medium()
        ...
        case .success(let downloadResult):
            BeansLogger.shared.log("下载完成：\(song.name)（\(quality.rawValue)\(downloadResult.downgraded ? "，已自动降级" : "")）", level: .info)
            if let recorded = downloadStore.addDownloadedSong(song: song, quality: quality.label, from: downloadResult.url, folderID: folderID) {
                let folderName = downloadStore.folders.first { $0.id == recorded.folderID }?.name ?? "文件夹"
                ToastCenter.shared.show("已保存到「\(folderName)」")
            } else {
                ToastCenter.shared.show("下载完成，保存失败")
            }
    ...
    private func handleSwipeEnd(height: CGFloat) {
        guard swipeSwitchSong, !showLyrics else { return }
        // 三层保险：歌词模式绝对不切歌
    ...
    private func flySwipe(direction: CGFloat) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.17) {
            ...
            // 双重保险：seek 后 1 秒内绝对不切歌（防止点按按钮被误判为滑动）
            guard Date().timeIntervalSince(player.lastSeekTime) > 1.0 else {
                withAnimation(.easeOut(duration: 0.2)) { swipeOffset = 0; coverDrag = .zero }
                return
            }
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack(alignment: .leading) {
                ...
                case 4:
                    // 霓虹双线：上下两条发光细线，已播放段双色渐变叠加
                    ZStack(alignment: .leading) {
                        Capsule()
                            .fill(track.opacity(0.4))
                            .frame(height: 2)
                        Capsule()
                            .fill(
                                LinearGradient(colors: [accent, .white.opacity(0.9), accent.opacity(0.55)],
                                               startPoint: .leading, endPoint: .trailing)
                            )
                    ...
                    ZStack(alignment: .leading) {
                        ...
                        Capsule()
                            .fill(
                                LinearGradient(colors: [accent.opacity(0.55), .white.opacity(0.85), accent],
                                               startPoint: .leading, endPoint: .trailing)
                            )
                            .frame(width: thumbX, height: 2)
                            .shadow(color: accent.opacity(0.8), radius: 6)
                    ...
                    .offset(y: 4)
                case 5:
                    // 能量分段：等距分段胶囊，已播放段逐段点亮并带亮度递进
                    HStack(spacing: 3) {
                        ForEach(0..<12, id: \.self) { seg in
                            let segRatio = Double(seg + 1) / 12.0
                            Capsule()
                                .fill(
                                    segRatio <= ratio
                                        ? (seg == min(Int(ratio * 12), 11) ? Color.white : accent.opacity(0.55 + Double(seg) * 0.04))
                                        : track.opacity(0.35)
                                )
                                .frame(height: 6)
                                .shadow(color: segRatio <= ratio ? accent.opacity(0.5) : .clear, radius: 3)
                        }
                    }
                case 6:
                    // 光谱渐变：多彩彩虹渐变，已播放段流动辉光
                    let spectrum = [Color(red: 0.95, green: 0.30, blue: 0.35), Color(red: 0.98, green: 0.72, blue: 0.25), Color(red: 0.45, green: 0.85, blue: 0.45), Color(red: 0.35, green: 0.80, blue: 0.95), Color(red: 0.62, green: 0.45, blue: 0.95)]
                    ZStack(alignment: .leading) {
                        Capsule()
                            .fill(track.opacity(0.4))
                            .frame(height: 6)
                        Capsule()
                            ...
                            .shadow(color: spectrum[1].opacity(0.6), radius: 5)
                    }
                case 7:
                    // 脉冲圆点：已播放段用跳动圆点排列
                    HStack(spacing: 4) {
                        ForEach(0..<20, id: \.self) { dot in
                            let dotRatio = Double(dot + 1) / 20.0
                            Circle()
                                .fill(dotRatio <= ratio ? accent : track.opacity(0.3))
                                .frame(width: 6, height: 6)
                                .scaleEffect(dotRatio <= ratio && player.isPlaying ? 1 + sin(Double(dot) * 0.8 + flowPhase * .pi * 2) * 0.3 : 1)
                                .shadow(color: dotRatio <= ratio ? accent.opacity(0.6) : .clear, radius: 3)
                        }
                    }
                    .frame(height: 10)
                case 8:
                    // 渐变光带：宽幅渐变 + 流动高光扫过
                    ZStack(alignment: .leading) {
                        Capsule().fill(track.opacity(0.4)).frame(height: 8)
                        Capsule()
                            .fill(LinearGradient(colors: [accent.opacity(0.7), accent, .white.opacity(0.9), accent],
                                                 startPoint: .leading, endPoint: .trailing))
                        ...
                        if player.isPlaying {
                            Capsule()
                                ...
                                .offset(x: max(0, min(thumbX - 40, thumbX * flowPhase)))
                        }
                    }
                case 9:
                    // 数字分段：等距方块，已播放段彩色递进
                    HStack(spacing: 2) {
                        ForEach(0..<16, id: \.self) { seg in
                            let segRatio = Double(seg + 1) / 16.0
                            RoundedRectangle(cornerRadius: 2)
                                .fill(segRatio <= ratio
                                    ? accent.opacity(0.4 + Double(seg) * 0.04)
                                    : track.opacity(0.25))
                                .frame(height: 8)
                        }
                    }
                case 10:
                    // 双轨镜像：上下镜像的两条进度线，中间细线连接
                    VStack(spacing: 3) {
                        Capsule().fill(track.opacity(0.4)).frame(height: 2)
                            .overlay(alignment: .leading) {
                                Capsule().fill(accent).frame(width: thumbX, height: 2).shadow(color: accent.opacity(0.7), radius: 4)
                            }
                        Capsule().fill(track.opacity(0.4)).frame(height: 2)
                            .overlay(alignment: .leading) {
                                Capsule().fill(accent.opacity(0.6)).frame(width: thumbX, height: 2)
                            }
                    }
                default:
    ...
    private func lyricRow(index: Int, line: LyricLine) -> some View {
        ...
        return VStack(spacing: 3) {
            Text(line.text.isEmpty ? " " : line.text)
                ...
                // 双层光晕：内层亮、外层宽，发光更明显
                .shadow(
                    color: isCurrent ? glowColor.opacity(glowRadius > 0 ? 0.7 : 0) : .clear,
                    radius: isCurrent ? glowRadius * 0.8 : 0
                )
...
struct LyricPreset {
    ...
    static let all: [LyricPreset] = [
        ...
        LyricPreset(name: "珊瑚海", start: "#FF8A80", end: "#FF5252", glow: 2),
        LyricPreset(name: "星空蓝", start: "#90CAF9", end: "#1565C0", glow: 3),
        LyricPreset(name: "抹茶绿", start: "#C5E1A5", end: "#558B2F", glow: 1),
        LyricPreset(name: "玫瑰金", start: "#FFCDD2", end: "#E57373", glow: 2),
        LyricPreset(name: "赛博朋克", start: "#00E5FF", end: "#D500F9", glow: 4),
        LyricPreset(name: "日落橙", start: "#FFCC80", end: "#FF6D00", glow: 2),
        LyricPreset(name: "薰衣草", start: "#D1C4E9", end: "#7E57C2", glow: 2),
        LyricPreset(name: "极光绿", start: "#69F0AE", end: "#00C853", glow: 3),
        LyricPreset(name: "蜜桃粉", start: "#FFAB91", end: "#FF5722", glow: 1),
        LyricPreset(name: "深海蓝", start: "#4DD0E1", end: "#006064", glow: 3),
        LyricPreset(name: "香槟金", start: "#FFE082", end: "#FF8F00", glow: 2),
        LyricPreset(name: "紫罗兰", start: "#CE93D8", end: "#4A148C", glow: 3),
        LyricPreset(name: "翡翠绿", start: "#80CBC4", end: "#004D40", glow: 2),
        LyricPreset(name: "烈焰红", start: "#EF9A9A", end: "#B71C1C", glow: 3),
        LyricPreset(name: "月光银", start: "#ECEFF1", end: "#546E7A", glow: 2),
        LyricPreset(name: "海洋深", start: "#4FC3F7", end: "#01579B", glow: 3),
    ]
}
...
struct PlayerSettingsSheet: View {
            ...
            ScrollView {
                LazyVStack(spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(spacing: 4) {
                            Text("正在播放的歌曲名称")
                                .font(BeansFont.appFont(CGFloat(fontSize), .bold))
                                .foregroundStyle(currentColor.wrappedValue)
                                .multilineTextAlignment(.center)
                                .frame(maxWidth: .infinity)
                            Text("未播放的歌词行显示为暗色")
                                .font(BeansFont.appFont(CGFloat(fontSize - 2)))
                                .foregroundStyle(dimColor.wrappedValue.opacity(0.6))
                                .multilineTextAlignment(.center)
                                .frame(maxWidth: .infinity)
                        }
                        .padding(.vertical, 8)
                        ...
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                RoundedRectangle(cornerRadius: 4)
                                    ...
                                    .frame(height: 6)
                                RoundedRectangle(cornerRadius: 4)
                                    .fill(LinearGradient(colors: [Color.beansAmber, Color.beansAmber.opacity(0.7)], startPoint: .leading, endPoint: .trailing))
                                    .frame(width: geo.size.width * 0.4, height: 6)
                                Circle()
                                    .fill(Color.beansAmber)
                                    .frame(width: 14, height: 14)
                                    .offset(x: geo.size.width * 0.4 - 7)
                                    .shadow(color: Color.beansAmber.opacity(0.6), radius: 6)
                            }
                        }
                        .frame(height: 14)
                    }
                    ...
                    .background {
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
                            .fill(LinearGradient(colors: [Color.beansAmber.opacity(0.08), Color.purple.opacity(0.06)], startPoint: .topLeading, endPoint: .bottomTrailing))
                            .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).strokeBorder(Color.beansAmber.opacity(0.2), lineWidth: 0.8))
                    }
                    playingCard
                    lyricDisplayCard
                    lyricEffectCard
    ...
    private var playingCard: some View {
        settingCard("播放", isExpanded: $playbackExpanded) {
            ...
            CompactSettingGroup {
                ...
                Divider().opacity(0.35)
                settingToggle("黑胶唱盘样式", isOn: $vinylRecordStyle,
                              caption: "封面变为黑胶唱片旋转效果")
                Divider().opacity(0.35)
                settingToggle("触动反馈", isOn: Binding(
                    get: { UserDefaults.standard.bool(forKey: "beans.hapticEnabled") },
                    set: { UserDefaults.standard.set($0, forKey: "beans.hapticEnabled") }
                ), caption: "点击和滑动时的震动反馈")
                Divider().opacity(0.35)
                settingToggle("启动自动播放", isOn: Binding(
                    get: { UserDefaults.standard.bool(forKey: "beans.autoPlayOnStartup") },
                    set: { UserDefaults.standard.set($0, forKey: "beans.autoPlayOnStartup") }
                ), caption: "打开App自动继续播放上次歌曲")
                Divider().opacity(0.35)
                settingToggle("交叉淡入淡出", isOn: Binding(
                    get: { UserDefaults.standard.bool(forKey: "beans.crossfadeEnabled") },
                    set: { UserDefaults.standard.set($0, forKey: "beans.crossfadeEnabled") }
                ), caption: "切歌时淡入淡出，消除曲间静音")
                Divider().opacity(0.35)
                settingToggle("低流量模式", isOn: Binding(
                    get: { UserDefaults.standard.string(forKey: "beans.audioQuality") == "low" },
                    set: { UserDefaults.standard.set($0 ? "low" : "exhigh", forKey: "beans.audioQuality") }
                ), caption: "使用64kbps音质，节省流量")
            }
        }
    }
    ...
    private var progressStyleGrid: some View {
        let styles: [(Int, String, String)] = [
            ...
            (2, "极光", "sparkles"),
            (3, "波浪", "waveform"),
            (4, "霓虹", "bolt.horizontal"),
            (5, "能量", "chart.bar.fill"),
            (6, "光谱", "rainbow"),
            (7, "脉冲", "circle.grid.3x3.fill"),
            (8, "光带", "light.beacon.min"),
            (9, "数字", "square.grid.3x3.fill"),
            (10, "双轨", "lines.measurement.horizontal"),
        ]
...
extension PlayerView {
    private func generateShareCard() {
        guard let song = player.currentSong, !isGeneratingShareCard else { return }
        isGeneratingShareCard = true
        Task {
            if let card = await ShareCardGenerator.generateCard(song: song) {
                // 保存到临时文件，用文件URL分享（避免UIImage直接分享导致的闪退）
                let tempDir = FileManager.default.temporaryDirectory
                let fileURL = tempDir.appendingPathComponent("share_card_\(UUID().uuidString).png")
                if let data = card.pngData() {
                    do {
                        try data.write(to: fileURL)
                        await MainActor.run {
                            shareCardFileURL = fileURL
                            showShareCard = true
                            isGeneratingShareCard = false
                        }
                    } catch {
