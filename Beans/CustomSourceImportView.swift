...
""")
    ...
    private func parseSources(from data: Data) -> [ThirdPartySource]? {
        ...
        // 尝试解析为数组
        if let array = try? JSONDecoder().decode([ThirdPartySource].self, from: data) {
            return array
        }
        // 尝试解析为包含 sources 字段的对象
        if let dict = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
           let sourcesData = dict["sources"] as? [[String: Any]] {
                ...
                if let name = item["name"] as? String,
                   let template = item["template"] as? String {
                    let source = ThirdPartySource(
                        name: name,
                        template: template,
                        urlPath: item["urlPath"] as? String ?? "url",
